...
class NeRFGUI:
    def __init__(self, opt, trainer, train_loader=None, debug=True):
        ...
        self.use_negative_labels = False
        self.downscale = 1
        self.train_steps = 16
    ...
    def test_step(self):
        if self.need_update or self.spp < self.opt.max_spp:
            ...
            if self.need_update_user_inputs:
                ...
                user_inputs['point_labels'] = self.selected_point_label
    ...
    def register_dpg(self):
        ...
        with dpg.window(label="Control", tag="_control_window", width=300, height=200):
            ...
            with dpg.group(horizontal=True):
                dpg.add_text("Save trajactery: ")

                def callback_start_track(sender, app_data):
                    self.trainer.track_trajectory = True
                    
                def callback_save_trajectory(sender, app_data):
                    self.trainer.save_trajectory()

                dpg.add_button(label="start track", tag="_button_start_track", callback=callback_start_track)
                dpg.add_button(label="save trajectory", tag="_button_save_trajectory", callback=callback_save_trajectory)
                dpg.bind_item_theme("_button_start_track", theme_button)
                dpg.bind_item_theme("_button_save_trajectory", theme_button)
            with dpg.group(horizontal=True):
                dpg.add_text("Save points: ")
                def callback_save_3d_points(sender, app_data):
                    self.trainer.save_3d_points()
                    
                    
                def callback_set_negative_labels(sender, app_data):
                    if self.use_negative_labels:
                        self.use_negative_labels = False
                    else:
                        self.use_negative_labels = True

                dpg.add_checkbox(label="use negative labels", default_value=self.dynamic_resolution, callback=callback_set_negative_labels)

                dpg.add_button(label="save 3D points", tag="_button_save_3d_points", callback=callback_save_3d_points)
                dpg.bind_item_theme("_button_save_3d_points", theme_button)
            # train button
            if not self.opt.test:
        ...
        def callback_point_select(sender, app_data):
            ...
            self.selected_point_label = np.array([0.]) if self.use_negative_labels else np.array([1.])
        ...
        with dpg.handler_registry():
            ...
            if self.opt.gui_mouse_right_drag:
                dpg.add_mouse_drag_handler(button=dpg.mvMouseButton_Right, callback=callback_camera_drag_pan)
                dpg.add_mouse_wheel_handler(callback=callback_set_mouse_loc)
            else:
        ...
        dpg.create_viewport(title='SANeRF-HQ', width=self.W, height=self.H, resizable=False)
