...
class NeRFRenderer(nn.Module):
    ...
    def run(self, rays_o, rays_d, bg_color=None, perturb=False, cam_near_far=None, update_proposal=True, return_feats=0, return_mask=0, H=None, W=None, **kwargs):
        ...
        #         if prop_iter == 0:
        #             # uniform sampling
        #             mesh_bin = torch.linspace(
        #                 0, 1, self.opt.num_steps[prop_iter] + 1, device=device).unsqueeze(0)  # [1, T+1]
        #             mesh_bin = mesh_bin.expand(N, -1)  # [N, T+1]
        #             if perturb:
        #                 mesh_bin = mesh_bin + (torch.rand_like(mesh_bin) - 0.5) / \
        #                     (self.opt.num_steps[prop_iter])
        #                 mesh_bin = mesh_bin.clamp(0, 1)
        #         else:
        #             # pdf sampling
        #             mesh_bin = sample_pdf(
        #                 mesh_bin, mesh_weights, self.opt.num_steps[prop_iter] + 1, perturb).detach()  # [N, T+1]

        #         # [N, T+1] in [near, far]
        #         mesh_real_bins = spacing_fn_inv(s_nears * (1 - mesh_bin) + s_fars * mesh_bin)

        #         mesh_rays_t = (mesh_real_bins[..., 1:] + mesh_real_bins[..., :-1]) / 2  # [N, T]

        #         mesh_xyzs = rays_o.unsqueeze(
        #             1) + rays_d.unsqueeze(1) * mesh_rays_t.unsqueeze(2)  # [N, T, 3]

        #         if self.opt.contract:
        #             mesh_xyzs = contract(mesh_xyzs)
    
        #         if prop_iter != len(self.opt.num_steps) - 1:
        #             # query proposal density
        #             with torch.set_grad_enabled(update_proposal):
        #                 outputs = self.density(mesh_xyzs)  # [N, T]
        #                 mesh_sigmas = outputs['sigma']
        #                 mesh_geo_feat = outputs['geo_feat']
        #             if return_mask > 0 and (self.opt.mask_mlp_type == 'default' or self.opt.mask_mlp_type == 'lightweight_mask'):
        #                 mesh_masks = self.m_grid(mesh_xyzs, bound=self.bound)
        #         else:
        #             # last iter: query nerf
        #             dirs = rays_d.view(-1, 1, 3).expand_as(mesh_xyzs)  # [N, T, 3]
        #             dirs = dirs / torch.norm(dirs, dim=-1, keepdim=True)
        #             mesh_outputs = self(mesh_xyzs, dirs, save_intermedian_results = self.opt.with_mask and self.opt.mask_mlp_type == 'adaptive')
        #             mesh_sigmas = mesh_outputs['sigma']
        #             mesh_colors = mesh_outputs['color']
        #             mesh_geo_feat = mesh_outputs['geo_feat']
        #             mesh_grid_output = mesh_outputs['grid_output']
        ...
        #         # sigmas to weights
        #         mesh_deltas = (mesh_real_bins[..., 1:] - mesh_real_bins[..., :-1])  # [N, T]
                
        #         m = torch.cat([mesh_masks, mesh_geo_feat.detach()], dim=-1)
        #         mesh_point_masks = self.mask_mlp(m)
                
        #         mesh_point_probability = F.softmax(mesh_point_masks, dim=-1)
        #         mesh_instance = mesh_point_probability.argmax(-1)
        #         mesh_mask = mesh_instance == self.opt.render_mask_instance_id
        #         mesh_sigmas = mesh_mask * mesh_sigmas
        #         mesh_deltas_sigmas = mesh_deltas * mesh_sigmas  # [N, T]

        #         # opaque background
        #         if self.opt.background == 'last_sample':
        #             mesh_deltas_sigmas = torch.cat(
        #                 [mesh_deltas_sigmas[..., :-1], torch.full_like(mesh_deltas_sigmas[..., -1:], torch.inf)], dim=-1)

        #         mesh_alphas = 1 - torch.exp(-mesh_deltas_sigmas)  # [N, T]
        #         mesh_transmittance = torch.cumsum(
        #             mesh_deltas_sigmas[..., :-1], dim=-1)  # [N, T-1]
        #         mesh_transmittance = torch.cat(
        #             [torch.zeros_like(mesh_transmittance[..., :1]), mesh_transmittance], dim=-1)  # [N, T]
        #         mesh_transmittance = torch.exp(-mesh_transmittance)  # [N, T]

        #         mesh_weights = mesh_alphas * mesh_transmittance  # [N, T]
        #         mesh_weights.nan_to_num_(0)
                
                
        #     mesh_weights = mesh_weights * mesh_mask
            
            
        #     mesh_weights_sum = torch.sum(mesh_weights, dim=-1)  # [N]
        #     depth = torch.sum(mesh_weights * mesh_rays_t, dim=-1)  # [N]
            
        #     if self.opt.sum_after_mlp:
        #         # pass the feature through view_mlp before getting the 
        #         f_colors = self.view_mlp(mesh_colors)
        #         f_colors_sum = torch.sum(mesh_weights.unsqueeze(-1) * f_colors, dim=-2)
        ...
        #     results['depth'] = depth
        #     m = torch.cat([mesh_masks, mesh_geo_feat.detach()], dim=-1)
        #     mask_point_masks = self.mask_mlp(m)
            
            
        #     mesh_point_logistics = mask_point_masks
        #     mesh_point_probability = F.softmax(mesh_point_logistics, dim=-1)
        #     mesh_instance = mesh_point_probability.argmax(-1)
        #     mesh_mask = mesh_instance == self.opt.render_mask_instance_id
                        
        #     mesh_image = image + (1 - mesh_weights_sum).unsqueeze(-1) * bg_color
        #     results['mesh_image'] = mesh_image

        #     if self.opt.render_mesh:
        #         results['instance_mask_logits'] = torch.sum(
        #         mesh_weights.detach().unsqueeze(-1) * mesh_point_masks, dim=-2)
        #     else:
        #         results['instance_mask_logits'] = torch.sum(
        #         weights.detach().unsqueeze(-1) * mesh_point_masks, dim=-2)
